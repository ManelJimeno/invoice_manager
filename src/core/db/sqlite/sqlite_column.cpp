...
SQLiteColumn::SQLiteDataType SQLiteColumn::fromSQLiteType(const QString& typeStr)
{
    static const QMap<QString, SQLiteDataType> typeMap = {
        {"INTEGER", SQLiteDataType::INTEGER}, {"REAL", SQLiteDataType::REAL},         {"TEXT", SQLiteDataType::TEXT},
        {"BLOB", SQLiteDataType::BLOB},       {"DATETIME", SQLiteDataType::DATETIME},
    };
...
SQLiteModifier SQLiteColumn::getModifierMask(const QStringList& modifiers)
{
    ...
    static const QMap<QString, SQLiteModifier> modifierMap = {{"is_primary_key", SQLiteModifier::isPrimaryKey},
...
QString SQLiteColumn::columnDefinition()
{
    ...
    QVector<std::function<QString()>> modifiers = {
        ...
        {[modifier = this->hasModifier(SQLiteModifier::isPrimaryKey)]() { return modifier ? " PRIMARY KEY" : ""; }},
